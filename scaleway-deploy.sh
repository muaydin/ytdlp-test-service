...
create_registry() {
    ...
    if scw registry namespace get $REGISTRY_NAME region=$REGION >/dev/null 2>&1; then
...
build_and_push_image() {
    ...
    REGISTRY_ENDPOINT=$(scw registry namespace get $REGISTRY_NAME region=$REGION --output=json | jq -r '.endpoint')
